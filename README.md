...
skill eligibility, and mustering out) for ten careers, each selected with its own
`-career` value: the Scout, the fixed-characteristic Rogue, the rankless Agent,
the auto-enrolling Citizen (benign Citizen Life), the Fame-driven Entertainer,
the Masterpiece-making Craftsman, the single-ladder Scholar (Research and
Publications), and three ranked armed-forces careers — the Soldier, Marine, and
Spacer — whose enlisted/officer ladders, Commissions, and Medal-boosted
promotions exercise the rank engine (e.g. `-career soldier`). The three remaining
careers (Merchant, Noble, Functionary), the rest of the education institutions,
and per-world orbital detail (systemgen) are the next stages.
