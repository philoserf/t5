...
func TestClimateCodes(t *testing.T) {
	...
	cases := []struct {
		...
		{"tropic (HZ-1) is also hot", temperate, 3, 4, true, []string{"Ho", "Tr"}},
		{"tundra (HZ+1) is also cold", temperate, 5, 4, true, []string{"Co", "Tu"}},
		{"HZ-1 with a non-tropic UWP is still hot", uwp.Profile{Size: 2, Atmosphere: 0}, 3, 4, true, []string{"Ho"}},
		{"HZ+1 with a non-tropic UWP is still cold", uwp.Profile{Size: 2, Atmosphere: 0}, 5, 4, true, []string{"Co"}},
		{"frozen (HZ+2)", uwp.Profile{Size: 5, Hydrographics: 6}, 6, 4, true, []string{"Fr"}},
		{"HZ+2 but no water", uwp.Profile{Size: 5, Hydrographics: 0}, 6, 4, true, nil},
		{"twilight orbit 0", temperate, 0, 4, true, []string{"Tz"}},
		{"twilight orbit 1", temperate, 1, 4, true, []string{"Tz"}},
		{"tropic and twilight", temperate, 1, 2, true, []string{"Ho", "Tr", "Tz"}},
		...
		// from).
		{"twilight without a habitable zone", temperate, 1, 0, false, []string{"Tz"}},
