...
func TestPassageFare(t *testing.T) {
	...
	for _, c := range cases {
		if got := PassageFare(c.class, c.demand); got != c.want {
			t.Errorf("PassageFare(%d, demand %d) = %d, want %d", c.class, c.demand, got, c.want)
		}
	}
}
...
func TestAvailableFreight(t *testing.T) {
	// (Flux + Pop + Liaison) x (valueTCs + 1). Regina-ish: pop 8, 3 value TCs.
	if got := AvailableFreight(0, 8, 3, 0); got != 32 { // 8 x (3+1)
		t.Errorf("AvailableFreight(0,8,3,0) = %d, want 32", got)
	}
	if got := AvailableFreight(2, 8, 3, 2); got != 48 { // (2+8+2) x 4
		t.Errorf("AvailableFreight(2,8,3,2) = %d, want 48", got)
	}
	if got := AvailableFreight(-9, 1, 5, 0); got != 0 { // floored at zero
