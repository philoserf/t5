...
`dice` (rolls, Flux/Good/Bad Flux, even distributions, roll-low Check/Resolve, notation
parser) · `ehex` · `uwp` · `worldgen` (full mainworld record — UWP + UWP-determinable trade
classifications + {Ix}(Ex)[Cx] Extensions + Bases/Nobility/Travel-Zone/Native-Status + PBG +
Second Survey line, golden-tested against Regina) · `systemgen` (stars: spectral
type/decimal/size, gas giants, planetoid belts, world count, secondary stars in orbit bands,
mainworld via worldgen; Second Survey system line) · `chargen` (full character life-cycle:
six-characteristic UPP + Check, homeworld skills, aging, pre-career education
[ED5/Trade/College/University/Masters/Professors], all 13 careers on a shared term-engine
[ranks, Branch/Operations, muster-DM, multi-career, first-career Retry + Citizen fallback] and
mustering-out, rendered as a labeled character sheet) · `skill` (skills & knowledges model) ·
`task` (Universal Task Format / Difficulty ladder) · `calendar` (Imperial Calendar).
...
**Done:** worldgen's census line (trade classes → PBG + {Ix}(Ex)[Cx] → Second Survey),
the Difficulty/UTF task layer, the Imperial Calendar, and the flagship `chargen careers`.
**Remaining critical path:** `RangeBand` (#9) → the play side it unlocks (Senses, Personals,
Combat); `systemgen per-world placement` (#15, which also unblocks the climate trade codes);
and `starship design` (#16, Book 2). Everything in Tiers 5–6 is independent
content-generation that can follow in any order.
...
**Done (2026-07-12):** all 13 careers on the shared term-engine (rank ladders, Branch/Operations,
generalized muster-DM, +1 commission/promotion skill), every career's signature mechanic, homeworld
skills, aging, the pre-career education ladder (ED5/Trade/College/University/Masters/Professors),
multi-career with first-career Retry + Citizen fallback, and a labeled character sheet — each a
golden-tested PR. **Deferred tail:** professional/military education (Honors→Medical/Law, Service
Academy, OTC/NOTC, Flight School — grant a pre-career commission/branch), the Entertainer-specialty
and full Citizen-Life-Skills tables, and Sophont support (#17) for alien PCs.
